...
extension String {
    func vector() -> Vector? {
        guard let dir = Direction(rawValue: first!) else { return nil }
        guard let magnitude = Int(String(dropFirst())) else { return nil }
        return Vector(direction: dir, magnitude: magnitude)
    }
}

struct Path {
    var currentPosition: Point = .centralPort
    var visitedPoints = Set<Point>()
    
    init(moves: [Vector]) {
        moves.forEach { move(with: $0) }
    }
    
    mutating func move(with vector: Vector) {
        switch vector.direction {
        case .up:
            (1...vector.magnitude).forEach { _ in
                currentPosition.y += 1
...
let path1 = Path(moves: input1.compactMap { $0.vector() })
let path2 = Path(moves: input2.compactMap { $0.vector() })
    
let commonPoints = path1.visitedPoints.intersection(path2.visitedPoints)

let part1 = commonPoints.sorted().first?.distance
